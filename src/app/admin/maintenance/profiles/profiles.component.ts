...
export class ProfilesComponent implements OnInit {
  ...
  getErrorMessage(controlName: string) {
    ...
    if (control?.hasError('required')) {
      return controlName === 'description'
        ? this.translate.instant('PROFILE.ERROR_REQUIRED')
        : '';
    }
    if (control?.hasError('whitespace')) {
      return controlName === 'description'
        ? this.translate.instant('PROFILE.ERROR_WHITESPACES')
        : '';
    }
    if (control?.hasError('pattern')) {
      return controlName === 'description'
        ? this.translate.instant('PROFILE.ERROR_PATTERN')
        : '';
    }
          ...
          next: () => {
            ...
            Swal.fire({
              ...
              title: this.translate.instant(
                'ALERT_PROFILE.TITLE_DELETE_SUCCESS'
              ),
              text: this.translate.instant(
                'ALERT_PROFILE.MESSAGE_DELETE_SUCCESS'
              )
            });
            this.resetForm();
          },
  ...
  onToggleChange(event: MatSlideToggleChange) {
    ...
    if (!status) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          this.userService
            .disableProfile(status, this.profileIdSelected)
            .subscribe({
              next: response => {
                Swal.fire({
                  icon: 'success',
  ...
  updateProfile() {
    ...
    if (this.profileIdSelected) {
      this.userService
        .updateProfile(data, this.profileIdSelected, roles)
        .subscribe({
          next: response => {
            Swal.fire({
              title: this.translate.instant('ALERT_PROFILE.TITLE_UPDATE'),
              text: this.translate.instant(
                'ALERT_PROFILE.MESSAGE_UPDATE_SUCCESS'
              ),
              icon: 'success'
            });
            this.obtenerPerfiles(this.currentPage, this.pageSize);
  ...
  searchProfile() {
    ...
    this.userService
      .searchProfile(description, this.currentPage, this.pageSize)
      .subscribe({
        next: response => {
          response.profiles.forEach((profile: any) => {
            profile.statusIcon = profile.status ? 'circle' : 'circle';
